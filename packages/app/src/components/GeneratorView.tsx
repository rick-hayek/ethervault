...
export const GeneratorView: React.FC = () => {
  ...
  const CompactOption = ({ icon: Icon, label, value, onClick }: any) => (
    ...
    >
      ...
      <div className={`w-8 h-4 rounded-full relative transition-colors duration-200 ${value ? 'bg-primary-600' : 'bg-slate-200 dark:bg-slate-700'
        }`}>
          ...
          <div className="lg:col-span-7 flex flex-col">
            <div className="bg-white dark:bg-slate-900 p-6 md:p-10 rounded-[32px] border border-slate-200 dark:border-slate-800 shadow-sm relative overflow-hidden flex-1 flex flex-col justify-center min-h-[300px]">
              ...
              <div className="relative text-center space-y-8 z-10">
                <span className="text-3xl md:text-5xl font-mono font-bold text-slate-800 dark:text-white line-clamp-2 break-all w-full px-4 tracking-tight leading-tight selection:bg-primary-500 selection:text-white min-h-[1.2em]">
                  {password}
                </span>
                ...
                <div className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto w-full pt-4">
                  ...
                  <button
                    onClick={handleCopy}
                    className="flex-1 py-4 bg-primary-600 text-white rounded-2xl font-medium shadow-lg shadow-primary-500/30 hover:bg-primary-700 transition-all flex items-center justify-center gap-2 active:scale-[0.98]"
                  >
                ...
                <div>
                  <div className="flex justify-between items-center mb-6">
                    ...
                    <input
                      ...
                      className="text-lg font-medium text-slate-900 dark:text-white bg-slate-100 dark:bg-slate-800 px-3 py-2 rounded-2xl tabular-nums w-20 text-center border border-slate-200 dark:border-slate-700 focus:outline-none focus:ring-2 focus:ring-primary-500/50"
                    />
                  </div>
                  ...
                  <div className="h-10 flex items-center px-1">
                    <input
                      ...
                      className="w-full h-3 bg-slate-100 dark:bg-slate-800 rounded-full appearance-none cursor-pointer accent-primary-600 hover:accent-primary-500 transition-all touch-none"
                    />
                  </div>
                </div>
