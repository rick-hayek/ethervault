    ...
    <div className="min-h-full">
      ...
      <div className="hidden md:flex flex-col gap-6 sticky top-0 z-20 bg-slate-50/95 dark:bg-slate-950/95 backdrop-blur-sm px-8 pt-8 pb-4 transition-all">
        <div className="flex flex-row items-center justify-between">
          ...
          <button
            onClick={onAdd}
            className="flex items-center justify-center gap-2 bg-slate-900 dark:bg-white text-white dark:text-slate-900 px-5 py-2.5 rounded-xl font-medium text-sm shadow-lg hover:opacity-90 active:scale-95 transition-all"
          >
            ...
            <div className="flex gap-1 overflow-x-auto w-full lg:w-auto p-1 scrollbar-hide">
              {CATEGORIES.map((cat) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-xl text-[10px] font-medium uppercase tracking-wider whitespace-nowrap transition-all ${activeCategory === cat
          ...
          {[...passwords].sort((a, b) => a.title.localeCompare(b.title)).map((entry) => (
            <React.Fragment key={entry.id}>
              ...
              >
                <div className="flex items-start justify-between mb-5">
                  <div className="flex-1 min-w-0 flex items-center gap-4">
                    <div className="w-12 h-12 rounded-2xl bg-slate-50 dark:bg-slate-800 flex items-center justify-center text-slate-900 dark:text-slate-100 font-medium border border-slate-100 dark:border-slate-700 overflow-hidden shadow-sm group-hover:shadow-md transition-all shrink-0">
                      {entry.icon ? (
                ...
                <div className="space-y-3">
                  <div className="p-3 bg-slate-50 dark:bg-slate-950/50 rounded-xl flex items-center justify-between group/row border border-slate-100 dark:border-slate-800/50 hover:border-slate-200 dark:hover:border-slate-700 transition-colors">
                    <div className="truncate pr-2 flex items-center gap-2">
                      ...
                      <span className="text-sm font-normal text-slate-700 dark:text-slate-200 truncate block font-mono tracking-tight">{entry.username}</span>
                    </div>
                    <button
                  ...
                  <div className="flex items-center justify-between pt-2 px-1">
                    {(() => {
                      ...
                      return (
                        <div className={`text-[9px] font-medium px-2.5 py-1 rounded-lg uppercase tracking-widest ${getStrengthColor(strength)}`}>
              ...
              <div className="md:hidden relative rounded-[20px] shadow-sm border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 overflow-hidden active:scale-[0.99] transition-all duration-200">
                ...
                >
                  {/* Icon */}
                  <div className="w-12 h-12 rounded-2xl bg-slate-50 dark:bg-slate-800 flex items-center justify-center text-slate-900 dark:text-slate-100 font-medium shrink-0 border border-slate-100 dark:border-slate-700 overflow-hidden shadow-sm">
                    {entry.icon ? (
                      <img src={entry.icon} alt={entry.title} className="w-full h-full object-cover" />
                    ) : (
                      <span className="text-lg">{entry.title.charAt(0).toUpperCase()}</span>
